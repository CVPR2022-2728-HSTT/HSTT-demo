...
import sys, os
sys.path.insert(0, '../')
from src.modeling.e2e_model import HSTT
from src.modeling.modeling import HSTTForSequenceClassification
...
def setup_model(cfg, device=None):
    ...
    transformer_model_cls = HSTTForSequenceClassification
...
def validate(model, val_loader, cfg, train_global_step, eval_score=True):
    ...
    if eval_score:
        ...
        if cfg.all_metric:
            vqa_scores = val_loader.dataset.evaluate_all_metric(qa_results)
...
def start_inference(cfg):
    ...
    inference_res_dir = join(
        ...
        f"step_{cfg.inference_model_step}_{cfg.inference_n_clips}_{cfg.score_agg_func}_{'txt' if cfg.inference_metric else 'csv'}"
    )
